        ...
        <div class="card-head m-3">
            <div class="row">
                ...
                <div class="col-md-6 d-flex justify-content-end ">
                    <a href="{{route('kk.create')}}" title="tambah data" class="btn btn-info">
                        <em class="ft-plus"></em>
                    </a>
                </div>
            </div>
                ...
                <tbody>
                    @foreach($collection as $key=>$item)
                        <tr>
                            ...
                            <td>{{$item->nomor}}</td>
                            <td>
                                @foreach ($dataUser->where('id', $item->user_id) as $item2)
                                    {{$item2->name}}
                                @endforeach
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item" href="{{route('kk.edit', $item->id)}}">Edit</a>
                                        <a class="dropdown-item" href="{{route('kk.show', $item->id)}}">Lihat Detail</a>
                                        <a class="dropdown-item" onclick="event.preventDefault(); document.getElementById('destroy-form').submit();">Hapus</a>
                                        <form id="destroy-form" action="{{ route('kk.destroy', $item->id) }}" method="POST" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                          </form>
                                    </div>
