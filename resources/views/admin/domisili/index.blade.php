                ...
                @foreach ($collection as $item)
                    <tr>
                        ...
                        <td>
                            ...
                            @else 
                                <a href="{{route('domisilis.edit', $item->id)}}" class="btn btn-sm btn-danger">Belum ACC</a>
                            @endif
                        </td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <a class="dropdown-item" href="{{ route('domisilis.show', $item->id) }}">Lihat Detail</a>
                                    <a class="dropdown-item"
                                            onclick="event.preventDefault(); document.getElementById('destroy-form').submit();">Hapus</a>
                                        <form id="destroy-form" action="{{ route('domisilis.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @if ($item->status == 0)
                                        {{-- <a class="dropdown-item" href="{{ route('domisilis.edit', $item->id) }}">Edit</a> --}}
                                        <a class="dropdown-item"
                                            onclick="event.preventDefault(); document.getElementById('destroy-form').submit();">Hapus</a>
                                        <form id="destroy-form" action="{{ route('domisilis.destroy', $item->id) }}" method="POST"
                                            class="d-none">
