        ...
        <div class="card-head m-3">
            <div class="row">
                ...
                <div class="col-md-6 d-flex justify-content-end ">
                    <a href="{{route('kades.create')}}" title="tambah data" class="btn btn-info">
                        <em class="ft-plus"></em>
                    </a>
                </div>
            </div>
                ...
                <tbody>
                    @foreach($collection as $key=>$a)
                        <tr>
                            ...
                            <td>{{$a->alamat}}</td>
                            <td>
                                <img src="{{asset('uploads/kades/'.$a->ttd)}}" alt="{{$a->ttd}}" width="50px">
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item" href="{{route('kades.edit', $a->id)}}">Edit</a>
                                        {{-- <a class="dropdown-item" href="{{route('kades.show', $a->id)}}">Lihat Detail</a> --}}
                                        {{-- <a class="dropdown-item" onclick="event.preventDefault(); document.getElementById('destroy-form').submit();">Hapus</a>
                                        <form id="destroy-form" action="{{ route('kades.destroy', $a->id) }}" method="POST" class="d-none">
                                            @csrf
                                            @method('DELETE')
                                          </form> --}}
                                    </div>
